...
test("a skills list scopes skill loading without losing the enable flag", async () => {
  const cwd = await mkdtemp(join(tmpdir(), "pi-web-subagents-skills-"));
  try {
    await mkdir(join(cwd, ".pi", "agents"), { recursive: true });
    await writeFile(
      join(cwd, ".pi", "agents", "scoped-skills.md"),
      "---\ndescription: Scoped skills\nskills: [code-review, testing]\n---\nReview.\n",
    );
    const scoped = resolveSubagentProfile(cwd, "scoped-skills");
    assert.equal(scoped.loadSkills, true);
    assert.deepEqual(scoped.skills, ["code-review", "testing"]);

    await writeFile(
      join(cwd, ".pi", "agents", "all-skills.md"),
      "---\ndescription: All skills\nskills: true\n---\nReview.\n",
    );
    const all = resolveSubagentProfile(cwd, "all-skills");
    assert.equal(all.loadSkills, true);
    assert.equal(all.skills, undefined);

    await writeFile(
      join(cwd, ".pi", "agents", "both-flags.md"),
      "---\ndescription: Both flags\nload_skills: true\nskills: [code-review]\n---\nReview.\n",
    );
    const both = resolveSubagentProfile(cwd, "both-flags");
    assert.equal(both.loadSkills, true);
    assert.deepEqual(both.skills, ["code-review"]);

    // "none" must not read as unbounded, or it would do the opposite of "[]".
    await writeFile(
      join(cwd, ".pi", "agents", "no-skills.md"),
      "---\ndescription: No skills\nskills: none\n---\nReview.\n",
    );
    assert.equal(resolveSubagentProfile(cwd, "no-skills").loadSkills, false);
    assert.deepEqual(resolveSubagentProfile(cwd, "no-skills").skills, []);

    await writeFile(
      join(cwd, ".pi", "agents", "empty-skills.md"),
      "---\ndescription: Empty skills\nskills: []\n---\nReview.\n",
    );
    assert.equal(resolveSubagentProfile(cwd, "empty-skills").loadSkills, false);
    assert.deepEqual(resolveSubagentProfile(cwd, "empty-skills").skills, []);

    await writeFile(
      join(cwd, ".pi", "agents", "star-skills.md"),
      "---\ndescription: Star skills\nskills: '*'\n---\nReview.\n",
    );
    assert.equal(resolveSubagentProfile(cwd, "star-skills").skills, undefined);
  } finally {
    await rm(cwd, { recursive: true, force: true });
  }
});
...
test("subagent resource snapshots keep the declared skill scope", () => {
  const entries = [{
    type: "custom",
    customType: SUBAGENT_META_TYPE,
    data: {
...
test("a saved skill scope survives the round trip", async () => {
  const cwd = await mkdtemp(join(tmpdir(), "pi-web-subagents-"));
  try {
    const saved = saveProjectSubagentProfile(cwd, profile({
      loadSkills: true,
      skills: ["code-review", " code-review ", "testing"],
    }));
    const source = await readFile(join(cwd, ".pi", "agents", "test-agent.md"), "utf8");
    const loaded = resolveSubagentProfile(cwd, "test-agent");

    assert.deepEqual(saved.skills, ["code-review", "testing"]);
    assert.deepEqual(loaded.skills, ["code-review", "testing"]);
    assert.match(source, /skills:/);
    assert.doesNotMatch(source, /^skills: true$/m);

    saveProjectSubagentProfile(cwd, { ...loaded, skills: ["testing"] });
    assert.deepEqual(resolveSubagentProfile(cwd, "test-agent").skills, ["testing"]);

    saveProjectSubagentProfile(cwd, { ...resolveSubagentProfile(cwd, "test-agent"), skills: [] });
    assert.deepEqual(resolveSubagentProfile(cwd, "test-agent").skills, []);

    const unscoped = saveProjectSubagentProfile(cwd, profile({ name: "plain", loadSkills: true }));
    assert.equal(unscoped.skills, undefined);
    assert.equal(resolveSubagentProfile(cwd, "plain").skills, undefined);
    assert.match(await readFile(join(cwd, ".pi", "agents", "plain.md"), "utf8"), /^skills: true$/m);
  } finally {
    await rm(cwd, { recursive: true, force: true });
  }
});
test("a malformed persisted skill scope fails closed instead of widening", () => {
  ...
  assert.equal(readSubagentSessionResources(snapshot({})).skills, undefined);
  for (const skills of ["private-skill", [42], [""], {}]) {
    assert.throws(() => readSubagentSessionResources(snapshot({ skills })), /Invalid subagent skill scope/);
  }
  assert.deepEqual(readSubagentSessionResources(snapshot({ skills: [] })).skills, []);
});

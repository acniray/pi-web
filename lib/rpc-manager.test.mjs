...
test("RPC session startup preloads extension-registered providers before restoring models", async () => {
  ...
  assert.ok(startupSource.indexOf("createAgentSessionServices(") < startupSource.indexOf("createAgentSessionFromServices("));
...
test("exact prompts are sent through before_agent_start instead of the SDK prompt state", async () => {
  ...
  // The composed prompt is a getter, not a value: fix for skills being resolved
  // once at creation and then frozen for the life of the session.
  assert.match(subagentSource, /exactSystemPromptRef\.current = \(\) => composeSubagentExactPrompt\(/);
  // What gets persisted is the profile body, never the rendered skill text: a
  // resumed session must follow edits to SKILL.md and must not grow the session
  // file by the size of every skill it loaded.
  const snapshotSource = subagentSource.slice(
    subagentSource.indexOf("resourceSnapshot: {"),
    subagentSource.indexOf("sessionManager.appendCustomEntry(SUBAGENT_META_TYPE"),
  );
  assert.ok(snapshotSource.length > 0, "the persisted resource snapshot must stay a single block");
  assert.match(snapshotSource, /exactSystemPrompt: promptPlan\.exactSystemPrompt/);
  assert.doesNotMatch(snapshotSource, /exactSystemPromptRef/);
  // A restored profile resolves its skills through the same helper as a fresh one.
  assert.match(startupSource, /subagentResources\?\.exactSystemPrompt !== undefined\s*\?\s*\(\) => composeSubagentExactPrompt\(/);
  assert.match(startupSource, /subagentSkillPromptText\(\{[\s\S]*?skills: inner\.resourceLoader\.getSkills\(\)\.skills/);
  assert.match(subagentSource, /createSubagentSkillPreloadExtension\(\(\) => skillPreloadRef\.current\?\.\(\)\)/);
  assert.match(subagentSource, /profile\.loadSkills && profile\.skills !== undefined && profile\.promptMode !== "replace"/);
  assert.match(startupSource, /usesSubagentSkillPreload/);
